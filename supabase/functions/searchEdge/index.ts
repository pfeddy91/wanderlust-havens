...
const MY_DEPLOY_VERSION = "searchEdge-v1.0.0"; // Make this unique!
console.log(`[VERSION CHECK - searchEdge] Function Loaded: ${MY_DEPLOY_VERSION}`);
...
serve(async (req) => {
  ...
  try {
    ...
    if (queryEmbedding.length > 0 && queryEmbedding.length !== EXPECTED_EMBEDDING_DIMENSION) {
        console.warn(`CRITICAL: Query embedding dimension (${queryEmbedding.length}) does not match expected tour embedding dimension (${EXPECTED_EMBEDDING_DIMENSION}); vector similarity will be incorrect.`);
